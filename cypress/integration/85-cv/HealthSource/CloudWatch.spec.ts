...
describe('Cloud watch health source', () => {
  ...
  it('should add cloud watch health source, if correct values are given', () => {
    ...
    cy.get('input[value="ACT_WHEN_HIGHER"]').should('be.checked')
    ...
    cy.findByText(/Metric name must be unique./).should('exist')
    ...
    cy.wait('@monitoredServicePostCall').then(intercept => {
      ...
      expect(sources?.healthSources?.[0]?.spec?.metricDefinitions?.[0]?.identifier).equals('CustomMetric 1')

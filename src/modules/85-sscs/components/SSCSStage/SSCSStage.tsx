...
import { SSCSComponentMounter } from '../SSCSApp/SSCSApp'

const getStageAttributes = (_getString: UseStringsReturn['getString']): StageAttributes => ({
  name: _getString('common.sscsText'),
  type: StageType.SSCS,
  icon: 'sscs-main',
...
const SSCSStage = (props: PipelineStageProps): React.ReactElement => (
  <SSCSComponentMounter<PipelineStageProps> component="SSCSStage" childProps={props} />
)
...
const getStageEditorImplementation = (isEnabled: boolean, _getString: UseStringsReturn['getString']): JSX.Element => (
  <SSCSStage
    name={_getString('common.sscsText')}
    type={StageType.SSCS}
    title={_getString('sscs.stageTitle')}
    description={_getString('sscs.stageDescription')}
    icon="sscs-main"
    hoverIcon="sscs-main"
    isHidden={!isEnabled}
    isDisabled={false}
...
stagesCollection.registerStageFactory(StageType.SSCS, getStageAttributes, getStageEditorImplementation)

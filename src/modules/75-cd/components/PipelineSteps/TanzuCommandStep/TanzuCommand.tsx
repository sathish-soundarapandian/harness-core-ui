              ...
              store: Yup.object().shape({
                ...
                spec: Yup.object().shape({
                  files: Yup.string()
                    .trim()
                    .required(
                      getString('common.validation.fieldIsRequired', { name: getString('common.git.filePath') })
                    )
                })
              })
            })
...
const TanzuCommandInputStep: React.FC<TanzuCommandProps> = ({
  ...
  return (
    <>
      ...
      {getMultiTypeFromValue(inputSetData?.template?.spec?.script?.store?.spec?.files?.file) ===
...
export class TanzuCommandStep extends PipelineStep<TanzuCommandData> {
  ...
  renderStep(props: StepProps<TanzuCommandData>): JSX.Element {
    ...
    if (this.isTemplatizedView(stepViewType)) {
      ...
      if ((variablesData.spec.script.store as StoreConfig)?.spec.files?.file) {
        variablesData.spec['spec.script.store.spec.files'] = defaultTo(
          (variablesData.spec.script.store as StoreConfig)?.spec.files?.file,
          ''
        )
      }
      return (
  ...
  protected stepName = 'Tanzu Command Script'

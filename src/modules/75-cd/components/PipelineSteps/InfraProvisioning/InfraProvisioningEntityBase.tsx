...
import { Field, FormikContextType, FormikProps, useFormikContext } from 'formik'
import { AllowedTypes, Container, Formik, FormikForm, FormInput } from '@harness/uicore'
...
} from '../DeployEnvironmentEntityStep/types'

export interface InfraProvisioningEntityProps {
  initialValues: DeployEnvironmentEntityFormState
  path?: string
  readonly?: boolean
  onUpdate?: (data: DeployEnvironmentEntityFormState) => void
  onChange?: (data: DeployEnvironmentEntityFormState) => void
  allowableTypes: AllowedTypes
  provisionerSnippetLoading: boolean
  provisionerEnabled: any
  originalProvisioner: any
}

export const InfraProvisioningEntityBase = ({
  initialValues,
  onUpdate,
  onChange,
  ...
  provisionerEnabled,
  originalProvisioner
}: InfraProvisioningEntityProps): JSX.Element => {
  const {
    state: {
      pipelineView,
  ...
  const formikRef = useRef<FormikContextType<DeployEnvironmentEntityFormState>>()
  ...
  return (
    <Formik
      ...
      initialValues={getInitialValuesInCorrectFormat<InfraProvisioningEntityData, DeployEnvironmentEntityFormState>(
        initialValues,
        transformValuesFieldsConfig
      )}
      formName="infraProvisionerEntityBase"
      validate={(_values: DeployEnvironmentEntityFormState) => {
        const schemaValues = getFormValuesInCorrectFormat<
          DeployEnvironmentEntityFormState,
          InfraProvisioningEntityData
        >(_values, transformValuesFieldsConfig)
        onChange?.(schemaValues)
      }}
      onSubmit={(_values: DeployEnvironmentEntityFormState) => {
        const schemaValues = getFormValuesInCorrectFormat<
          DeployEnvironmentEntityFormState,
          InfraProvisioningEntityData
        >(_values, transformValuesFieldsConfig)
    ...
    >
      {(formik: FormikProps<DeployEnvironmentEntityFormState>) => {
        formik = useFormikContext<DeployEnvironmentEntityFormState>()
        console.log(formik)
        return (
          <FormikForm className={css.provisionerForm}>
            <FormInput.CheckBox
              name={`provisionerEnabled`}
              disabled={isProvisionerDisabled(provisionerSnippetLoading as boolean)}
              label={getString('pipelineSteps.deploy.provisioner.enableProvisionerLabel')}
              onChange={(event: React.FormEvent<HTMLInputElement>) => {
                if (!event.currentTarget.checked) {
                  formik.values.provisioner = { steps: [], rollbackSteps: [] }
                  formik.setFieldValue('provisioner', formik.values.provisioner)
                  onUpdate?.({
                    provisioner: formik.values.provisioner,
            ...
            />
            {provisionerSnippetLoading || stepsDataLoading ? (
              <Container>
                <Spinner />
              </Container>
            ) : provisionerEnabled ? (
                  ...
                  {(_props: any) => {
                    return (
                      <ExecutionGraph
                        ...
                        originalStage={originalProvisioner}
                        ref={executionRef}
...
export const InfraProvisioningEntityBaseWithRef = React.forwardRef(InfraProvisioningEntityBase)

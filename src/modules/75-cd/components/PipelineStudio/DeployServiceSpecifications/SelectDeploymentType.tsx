...
export default function SelectDeploymentType({
  ...
  const { SSH_NG, NG_SVC_ENV_REDESIGN, SPOT_ELASTIGROUP_NG, TAS_NG } = useFeatureFlags()

  // Supported in NG (Next Gen - The one for which you are coding right now)
  const ngSupportedDeploymentTypes = React.useMemo(() => {
    return getNgSupportedDeploymentTypes({ SSH_NG, NG_SVC_ENV_REDESIGN, SPOT_ELASTIGROUP_NG, TAS_NG })
  }, [SSH_NG, NG_SVC_ENV_REDESIGN, SPOT_ELASTIGROUP_NG, TAS_NG])

  ...
  const getStagesAllowedforPropogate = useCallback(
    (stageItem): boolean => {
      ...
      if (stageItem.stage.template) {
        ...
        const prevStageItemCustomDeploymentRef =
          // eslint-disable-next-line @typescript-eslint/ban-ts-comment
          // @ts-ignore
          // TODO: Fix once BE types are available
          (stageItem.stage as DeploymentStageElementConfig)?.spec?.customDeploymentRef
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-ignore
        // TODO: Fix once BE types are available
        const currentStageCustomDeploymentRef = stage?.stage?.spec?.customDeploymentRef
        ...
        const areDeploymentDetailsSame =
          currentStageDeploymentType === ServiceDeploymentType.CustomDeployment
            ? prevStageItemDeploymentType === currentStageDeploymentType &&
              isEqual(prevStageItemCustomDeploymentRef, currentStageCustomDeploymentRef)
            : prevStageItemDeploymentType === currentStageDeploymentType

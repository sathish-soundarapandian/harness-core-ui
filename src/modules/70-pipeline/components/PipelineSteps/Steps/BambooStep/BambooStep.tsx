...
import type { BambooStepSpec, BambooStepData } from './types'
import { variableSchema } from './helper'

const BambooStepInputSet = connect(BambooStepInputStep)
export interface BambooStepSpecUI
  extends Omit<BambooStepSpec, 'connectorRef' | 'tags' | 'labels' | 'buildArgs' | 'pull' | 'resources'> {
...
export interface BambooStepDataUI extends Omit<BambooStepData, 'spec'> {
  spec: BambooStepSpecUI
}
...
export class BambooStep extends PipelineStep<BambooStepData> {
  ...
  processFormData(data: any): BambooStepData {
    return {
      ...data,
      spec: {
        ...data.spec,

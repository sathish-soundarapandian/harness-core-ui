...
import { buildELKPayload, ElkAuthType } from '@connectors/pages/connectors/utils/ConnectorUtils';
...
function UsernamePasswordAndApiKeyOption(props: UsernamePasswordAndApiKeyOptionProps): JSX.Element {
	...
	const authOptions = useMemo(
		() => [
			{ label: getString('usernamePassword'), value: ElkAuthType.USERNAME_PASSWORD },
			{ label: getString('common.apikey'), value: ElkAuthType.API_CLIENT_TOKEN },
			{ label: getString('connectors.elk.noAuthentication'), value: ElkAuthType.NONE }
		],
		[]
	);
	...
	const fieldProps =
		authTypeValue === ElkAuthType.API_CLIENT_TOKEN
			? [
					{ name: 'apiKeyId', label: getString('connectors.elk.apiId'), key: 'apiKeyId' },
					{ name: 'apiKeyRef', label: getString('common.apikey'), key: 'apiKeyRef' }
			  ]
...
function ElkConfigStep(props: ConnectionConfigProps): JSX.Element {
	...
	const initialValues = initializeElkConnector({ prevStepData, projectIdentifier, accountId, orgIdentifier });
	...
	return (
		<>
			<StepDetailsHeader connectorTypeLabel={getString('connectors.elkLabel')} />
			<Formik
				enableReinitialize
				initialValues={{
					...initialValues

...
import { buildPcfPayload } from '@connectors/pages/connectors/utils/ConnectorUtils'
import { ConnectivityModeType } from '@common/components/ConnectivityMode/ConnectivityMode'
import ConnectorDetailsStep from '../commonSteps/ConnectorDetailsStep'
import StepPcfAuthentication from './StepAuth/StepPcfAuthentication'
import ConnectivityModeStep from '../commonSteps/ConnectivityModeStep/ConnectivityModeStep'

const PCFConnector: React.FC<CreateConnectorModalProps> = props => {
  const { onClose, onSuccess } = props
  const { getString } = useStrings()
  return (
    <StepWizard
      icon={getConnectorIconByType(Connectors.PCF)}
      iconProps={{ size: 37, inverse: true }}
      title={getString(getConnectorTitleIdByType(Connectors.PCF))}
    >
      <ConnectorDetailsStep
        ...
        disableGitSync={true}
        helpPanelReferenceId="PCFConnectorOverview" // validate
      />
      <StepPcfAuthentication
        ...
        {...props}
        connectorInfo={props.connectorInfo}
        helpPanelReferenceId="PCFConnectorCredentials" // validate
      />

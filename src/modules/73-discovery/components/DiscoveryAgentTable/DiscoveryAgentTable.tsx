...
import type { DatabaseInfraCollection } from 'services/servicediscovery'
import routes from '@common/RouteDefinitions'
import type { ProjectPathProps } from '@common/interfaces/RouteInterfaces'
import css from './DiscoveryAgentTable.module.scss'

interface DiscoveryAgentTableProps {
  list: DatabaseInfraCollection[]
}

const Name: Renderer<CellProps<DatabaseInfraCollection>> = ({ row }) => {
...
const NetworkCount: Renderer<CellProps<DatabaseInfraCollection>> = () => (
...
const ServiceCount: Renderer<CellProps<DatabaseInfraCollection>> = () => (
  <Layout.Vertical width={60} height={50} className={css.totalServiceContainer}>
    <Text font={{ size: 'medium', weight: 'semi-bold' }} color={Color.GREY_500}>
      10
    </Text>
...
const LastServiceDiscovery: Renderer<CellProps<DatabaseInfraCollection>> = ({ row }) => {
...
const LastModified: Renderer<CellProps<DatabaseInfraCollection>> = ({ row }) => (
  <Layout.Horizontal flex={{ align: 'center-center', justifyContent: 'flex-start' }}>
...
const ThreeDotMenu: Renderer<CellProps<DatabaseInfraCollection>> = () => (
...
const DiscoveryAgentTable: React.FC<DiscoveryAgentTableProps> = ({ list }) => {
  return (
    <TableV2<DatabaseInfraCollection>

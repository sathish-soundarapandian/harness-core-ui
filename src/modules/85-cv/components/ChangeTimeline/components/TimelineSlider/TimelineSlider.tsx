...
import { timeout } from '@cd/components/PipelineSteps/CommandScripts/__tests__/Props';
		...
		(parentClickEvent: MouseEvent): void => {
			/* istanbul ignore else */ if (mainRef.current) {
				...
				/* istanbul ignore else */ if (updatedSliderAspects) {
					...
					onSliderDragEnd?.(
						calculateSliderDragEndData(updatedSliderAspects, parentClickEvent, containerWidth)
					);
				}
			}
		},
			...
			>
				...
				<TimelineSliderHandle
					...
					onDrag={(e) => {
						...
						if (
							...
							setSliderAspects((currAspects) =>
								calculateSliderAspectsOnLeftHandleDrag(currAspects, draggableEvent)
							);
						}
					}}
				/>
				<Draggable
					...
					onDrag={(e) => {
						...
						if (isSliderWithinBounds({ draggableEvent, leftOffset, width, containerWidth })) {
							setSliderAspects((currAspects) =>
								calculateSliderAspectsOnDrag(currAspects, draggableEvent)
							);
						}
					}}
				>
				...
				<TimelineSliderHandle
					...
					onDrag={(e, dragData) => {
						...
						setSliderAspects((currAspects) =>
							calculateSliderAspectsOnRightHandleDrag(currAspects, dragData)
						);
					}}
				/>
			</Container>

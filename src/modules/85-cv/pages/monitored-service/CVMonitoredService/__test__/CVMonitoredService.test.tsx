...
jest.spyOn(cvServices, 'useGetMonitoredServiceListEnvironments').mockImplementation(
	() => ({ data: ['new_env_test', 'AppDTestEnv1', 'AppDTestEnv2'] } as any)
);
...
jest.spyOn(cvServices, 'useGetCountOfServices').mockImplementation(
	() => ({ data: serviceCountData, refetch: refetchServiceCountData } as any)
);
...
describe('Monitored Service list', () => {
	...
	test('delete flow works correctly', async () => {
		jest.spyOn(cvServices, 'useListMonitoredService').mockImplementation(
			() => ({ data: updatedMSListData, refetch: jest.fn() } as any)
		);

		jest.spyOn(cvServices, 'useGetCountOfServices').mockImplementation(
			() => ({ data: updatedServiceCountData, refetch: refetchServiceCountData } as any)
		);

...
import type {
  ...
  HealthSourceMetricThresholdType
...
export interface CommonHealthSourceInterface {
  // Fields coming from define health source screen
  name: string
  identifier: string
  connectorRef: { connector: { identifier: string }; value: string } | string
  product: string | SelectOption
  sourceType: ConnectorInfoDTO['type']

...
export const LoadSourceByType = ({
  ...
  onSubmit: (
    formdata: any,
    healthSourceList: UpdatedHealthSource
  ) => Promise<void>;
  isTemplate?: boolean;
  expressions?: string[];
}): JSX.Element | null => {
...
export const createHealthsourceList = (
  ...
  if (
    ...
    healthSources.some(
      (el: any) =>
        el?.identifier === healthSourcesPayload?.identifier &&
        el?.type === healthSourcesPayload?.type
    )
  ) {
    updatedHealthSources = healthSources?.map((el: any) =>
      el?.identifier === healthSourcesPayload?.identifier
        ? healthSourcesPayload
        : el
    );
